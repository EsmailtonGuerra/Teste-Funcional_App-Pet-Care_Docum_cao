...
create table alerta_campanhas (
    ...
    nome_campanha varchar(256),
	desc_campanha text not null,
    inicio_campanha datetime not null,
    fim_campanha datetime not null,
    fonte_web varchar(256),
    fk_id_clinica int not null,
    foreign key (fk_id_clinica) references clinica (id_clinica) ON DELETE CASCADE,
...
BEGIN
    ...
    IF NEW.tipo_user = 'Tutor' THEN
        ...
        VALUES (NEW.id_login, NULL, 'Nome Tutor', NULL, NULL, NULL, NEW.id_login); 
    ELSEIF NEW.tipo_user = 'Clinica' THEN
        ...
        VALUES (NEW.id_login, NULL, 'Nome Clínica', NULL, NEW.id_login, NULL, NULL, NEW.id_login);
	ELSEIF NEW.tipo_user = 'ADM' THEN
        INSERT INTO adm (id_adm, nome, fk_id_login)
        VALUES (NEW.id_login, 'Nome ADM', NEW.id_login);
    END IF;
...
INNER JOIN clinica c ON c.id_clinica = ac.fk_id_clinica
...
SELECT l.id_login, l.email, c.nome, c.cfmv_crmv, c.telefone
FROM login AS l
...
SELECT l.id_login, l.email, a.nome
FROM login AS l
